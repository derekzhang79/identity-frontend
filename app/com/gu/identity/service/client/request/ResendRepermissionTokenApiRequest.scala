...
case class ResendRepermissionTokenApiRequest(data: ResendTokenActionRequestBody)
  ...
  override val body = Some(ResendRepermissionFromTokenApiRequestBody(`token` = data.token))
...
case class ResendRepermissionFromTokenApiRequestBody(`token`: String) extends ApiRequestBody
