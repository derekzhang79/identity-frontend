...
import com.gu.identity.frontend.models.{ClientID, GroupCode, Referrer, ReturnUrl}
...
class Application(
  ...
  def register(error: Seq[String], returnUrl: Option[String], skipConfirmation: Option[Boolean], clientId: Option[String], group: Option[String], referrer: Option[String]) = (CSRFAddToken(csrfConfig) andThen MultiVariantTestAction) { implicit req =>
    val clientIdActual = ClientID(clientId)
    val returnUrlActual = ReturnUrl(returnUrl, req.headers.get("Referer"), configuration, clientIdActual)
    val referrerActual = Referrer(referrer)
    val csrfToken = CSRFToken.fromRequest(csrfConfig, req)
    val groupCode = GroupCode(group)
    val email : Option[String] = req.cookies.get("GU_SIGNIN_EMAIL").map(_.value)
    ...
    renderRegister(configuration, req.activeTests, error, csrfToken, returnUrlActual, skipConfirmation, clientIdActual, groupCode, email, referrerActual, shouldCollectConsents, shouldCollectV2Consents)
  }
