...
//object UserAuthenticatedAction extends ActionRefiner[Request, UserAuthenticatedRequest] with ActionBuilder[UserAuthenticatedRequest] with Logging {
//
//  override protected def refine[A](request: Request[A], identityCookieDecoder: IdentityCookieDecoder): Future[Either[Result, UserAuthenticatedRequest[A]]] = Future {
//
//    AuthenticationService.authenticatedUserFor(request, identityCookieDecoder.getUserDataForScGuU) match {
//      case Some(authenticatedUser) => {
//        getSC_GU_UCookie(request.cookies) match {
