import { EV_DONE, EV_START_OVER } from 'components/two-step-signin/two-step-signin__slide';
...
const pushSlide = ($old, $new, reverse = false) => {
  ...
  const classNames = reverse
    ? {in:'two-step-signin__slide--in-reverse',out:'two-step-signin__slide--out-reverse'}
  ...
  return new Promise((resolve, error)=> {
    ...
    requestAnimationFrame(() => {
      ['two-step-signin__slide--visible', classNames.in].forEach(_ => $new.classList.add(_))
    });
...
const initOnce = () => {
  window.addEventListener('popstate',(ev) => {
    if(ev.state && ev.state.initiator && ev.state.initiator === STATE_INITIATOR) {
      ev.preventDefault();
      ev.stopPropagation();
